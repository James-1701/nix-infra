# Aerothemeplasma KDE theme
# This is based off the work done by aean0x at: https://gitgud.io/aean0x/aerothemeplasma/-/tree/master
...
let
  themeRepo = fetchzip {
    ...
    sha256 = "sha256-fXxNDNm5dFRr5g3k0alEsoc83wyKMIt9Ud/yFOCT3II=";
  };
  ...
  decoration = stdenv.mkDerivation {
    ...
    buildInputs =
      qt6Deps ++ kf6Deps ++ kf6DevDeps ++ plasmaDeps ++ waylandDeps ++ [ pkgs.kdePackages.libplasma ];
    configurePhase = ''
  ...
  corebindingsplugin = stdenv.mkDerivation {
    ...
    inherit (pkgs.kdePackages.libplasma) src;
    nativeBuildInputs = commonNativeBuildInputs ++ [ pkgs.pkg-config ];
